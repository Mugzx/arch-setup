...
if snapper list-configs | grep -q "^root "; then
    ...
    if exe snapper -c root create --description "Before Setup" --cleanup-algorithm number; then
...
if snapper list-configs | grep -q "^home "; then
    ...
    if exe snapper -c home create --description "Before Setup" --cleanup-algorithm number; then
