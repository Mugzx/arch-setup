...
if [ -f "$LIST_FILE" ]; then
    ...
    if [ ${#REPO_APPS[@]} -gt 0 ]; then
        ...
        if [ ${#REPO_QUEUE[@]} -gt 0 ]; then
            # Batch Install
            exe runuser -u "$TARGET_USER" -- paru -S --noconfirm --needed --skipreview --nocleanafter --keepsrc --noremovemake "${REPO_QUEUE[@]}"
    ...
    if [ ${#AUR_APPS[@]} -gt 0 ]; then
        ...
        for aur_pkg in "${AUR_APPS[@]}"; do
            ...
            for (( i=0; i<=max_retries; i++ )); do
                ...
                runuser -u "$TARGET_USER" -- paru -S --noconfirm --needed --skipreview --nocleanafter --keepsrc --noremovemake "$aur_pkg"
                EXIT_CODE=$?
